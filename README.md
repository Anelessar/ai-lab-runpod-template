...
При старте Pod публичный порт `8188` сразу занимает лёгкий HTTP/WebSocket-шлюз. Поэтому кнопка `Port 8188 → ComfyUI` в RunPod доступна с первой секунды и показывает страницу ожидания, пока GPU инициализируется. Сам ComfyUI безопасно ждёт готовности CUDA на внутреннем порту `8189`; после запуска шлюз автоматически начинает передавать ему запросы без изменения публичного URL. Launcher и Jupyter при этом открываются независимо.
