...
ComfyUI ждёт реальной готовности CUDA, а затем сам занимает публичный порт `8188`. В цепочке нет дополнительного proxy-процесса: RunPod помечает `Port 8188 → ComfyUI` как Ready, когда сам интерфейс уже готов принимать запросы. Launcher и Jupyter запускаются независимо.
